...
class Sublanguage_current extends Sublanguage_core {
	...
	public function load() {
		...
		add_filter('sublanguage_untranslated_meta', array($this, 'untranslated_meta'), 10, 6); // -> @from 2.7
	...
	public function translate_home_url($url, $path, $orig_scheme, $blog_id) {
		...
		if (!$this->disable_translate_home_url
			...
			if (get_option('permalink_structure')) {
				...
				$url = rtrim(substr($url, 0, strlen($url) - strlen($path)), '/') . '/' . $language->post_name;
	...
	/**
	 * Translate meta. Public API
	 * Filter for 'sublanguage_untranslated_meta'
	 *
	 * @from 2.7
	 */
	public function untranslated_meta($meta_value, $meta_key, $post, $single = true, $language = null, $by = null) {

		if ($this->is_meta_key_translatable($post->post_type, $meta_key)) {

			$language = $this->find_language($language, $by);

			if ($language && $this->is_sub($language)) {

				$translated_value = $this->get_untranslated_post_meta($post->ID, $meta_key, $single);

				if ($translated_value) {

					$meta_value = $translated_value;
	...
	public function translate_custom_post_link($link, $post_id, $sample = false) {
		...
		if (!$sample) {
			...
			if ($post && $this->is_post_type_translatable($post->post_type) && !$draft_or_pending) {
